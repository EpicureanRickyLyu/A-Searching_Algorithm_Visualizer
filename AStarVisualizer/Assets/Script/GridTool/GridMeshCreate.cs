...
public class GridMeshCreate : MonoBehaviour 
{
    ...
    public void CreateMesh()
    {
        ...
        //初始化寻路数组
        ClearMesh();
        //分配Grid obj的数组空间，和初始化寻路数组空间
        AStarMgr.GetInstance().InitMapInfo(meshRange.horizontal,meshRange.vertical);
        m_grids = new GameObject[meshRange.horizontal, meshRange.vertical];
        m_starNode = new Grid[meshRange.horizontal,meshRange.vertical];
        for (int i = 0; i < meshRange.horizontal; i++)
        {
            for (int j = 0; j < meshRange.vertical; j++)
            {
                CreateGrid(i, j);
                m_starNode[i,j] = m_grids[i,j].GetComponent<Grid>();

            }
        }
        AStarMgr.GetInstance().InitAstarNode(m_starNode);
    }
